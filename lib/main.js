...
var prompts = Cc["@mozilla.org/embedcomp/prompt-service;1"].getService();
prompts = prompts.QueryInterface(Ci.nsIPromptService);
...
exports.main=function(options){
	...
	var menuItemAddTerm = cm.Item({
        ...
        onMessage: function(text) {
        	var newTerm = text.toLowerCase().trim();
        	var token = oauth.getToken();
        	...
        	if (!setId) {
        		...
        		request.Request({
        			url: "https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20150520T095829Z.0a7f4467b16aa50d.8cebb0ad825a5bda0dce118d9957014c025842c8&text=" + newTerm + "&lang=en-" + prefs.translationLanguage,
        			onComplete: function(response) {
	        			var sugested_translation = response.json.text[0].toLowerCase();
	        			var check = {value: false};               // default the checkbox to false
	    				var input = {value: sugested_translation};                  // default the edit field to Bob
	    				var translation = prompts.prompt(null, "Term translation", "Please, write translation", input, null, check);
	    				if (translation) {
	    					request.Request({
		        				...
		        				onComplete: function(response){
		        					...
		        					console.log("'"+newTerm+"'", terms);
		        					var termExist = false;
		        					for (var i = 0; i < terms.length; i++) {
										var term = terms[i];
										console.log("'"+term.term.toLowerCase()+"'");
										if (term.term.toLowerCase() == newTerm) {
											termExist = true;
											break;
										}
									}
		        					...
		        					if (!termExist) {
		        						request.Request({
					        				...
					        				content: {
					        					term: newTerm,
					        					definition: input.value.toLowerCase().trim() || 'undefined'
					        				},
					        				onComplete: function(response){
		        						...
		        						showInfoBox("term '" + newTerm + "' is already in the Set");
		        					}
		        				}
		        			}).get();
	    				}
